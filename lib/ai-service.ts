...
export async function generatePronunciation(
  word: string
): Promise<{ pronunciation: string; audioUrl: string }> {
  try {
...
export async function generateVocabularyWithAI(
  ...
  try {
    const apiKey = getGeminiApiKey();
    let prompt = `Generate ${count} vocabulary words at CEFR level ${level} based on Oxford Learner Dictionary 3000 words`;
